  ...
  <div>
    <el-card>
      ...
      <div class="search-bar">
        <el-input v-model="query.keyword" placeholder="搜索客户名称" clearable style="width: 300px" @clear="search" @keyup.enter="search" />
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑客户' : '新增客户'" width="550px" :close-on-click-modal="false">
...
import { getCustomerPage, createCustomer, updateCustomer, deleteCustomer, type Customer } from '../../api/customer'
...
const initForm = (): Partial<Customer> => ({ customerName: '', contactPerson: '', phone: '', address: '', remark: '', status: 1 })
const form = reactive<Partial<Customer>>(initForm())
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) { await updateCustomer({ ...form }); ElMessage.success('更新成功') }
